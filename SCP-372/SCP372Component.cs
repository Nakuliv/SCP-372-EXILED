    ...
    public class SCP372Component : MonoBehaviour
    {
        ...
        private IEnumerator<float> VoiceChatCheck()
        {
            while (true)
            {
                ...
                if (player.IsInvisible && player.IsVoiceChatting || player.IsInvisible && player.Radio.UsingRadio)
                {
                    player.IsInvisible = false;
                }
                else if (!player.IsInvisible && !player.IsVoiceChatting && !player.Radio.UsingRadio)
                {
